<script>
  ...
  onMount(() => {
    if ($authStore && $authStore.role === 'employee') {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        error = data.message || 'Invalid employee credentials';
      }
    } catch (err) {
...
<div class="min-h-screen relative overflow-hidden">
  ...
  <div class="absolute inset-0 bg-gradient-to-br from-emerald-900 via-green-900 to-teal-900">
    <!-- Animated Blobs -->
    <div class="absolute top-0 -left-4 w-72 h-72 bg-emerald-500 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"></div>
    <div class="absolute top-0 -right-4 w-72 h-72 bg-teal-500 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000"></div>
    <div class="absolute -bottom-8 left-20 w-72 h-72 bg-green-500 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-4000"></div>
    ...
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8 sm:mb-10">
        ...
        <div class="relative inline-block mb-6">
          <!-- Outer Glow Ring -->
          <div class="absolute inset-0 bg-gradient-to-r from-emerald-400 to-teal-400 rounded-full blur-2xl opacity-50 animate-pulse"></div>
          
          <!-- Logo Circle -->
          <div class="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto">
            <div class="absolute inset-0 bg-gradient-to-br from-emerald-400 via-green-500 to-teal-600 rounded-full animate-spin-slow"></div>
            <div class="absolute inset-1 bg-gradient-to-br from-emerald-900 to-teal-900 rounded-full flex items-center justify-center backdrop-blur-xl">
      ...
      <div class="bg-white/10 backdrop-blur-2xl rounded-3xl sm:rounded-[2rem] border border-white/20 shadow-2xl p-6 sm:p-8 relative overflow-hidden">
        ...
        <form on:submit|preventDefault={handleLogin} class="space-y-5 relative z-10">
          ...
          <div class="flex items-center justify-between pt-1">
            <label class="flex items-center group cursor-pointer">
              <input
                type="checkbox"
                class="w-4 h-4 text-emerald-500 bg-white/10 border-white/30 rounded focus:ring-emerald-500 focus:ring-2 cursor-pointer"
              />
              <span class="ml-2 text-sm text-white/80 group-hover:text-white transition">Remember me</span>
            </label>
            <button type="button" class="text-sm text-emerald-300 hover:text-emerald-200 transition font-medium">
              Forgot password?
            </button>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            class="w-full mt-6 py-4 bg-gradient-to-r from-emerald-500 via-green-500 to-teal-600 hover:from-emerald-600 hover:via-green-600 hover:to-teal-700 disabled:from-gray-600 disabled:to-gray-700 text-white font-bold rounded-2xl shadow-xl hover:shadow-2xl transition-all text-base disabled:cursor-not-allowed transform hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden group"
          >
            <!-- Button Shine Effect -->
        ...
        <div class="mt-6 pt-6 border-t border-white/10 relative z-10">
          ...
          >
            Are you an admin? <span class="font-bold text-white">Sign in as Admin</span>
          </button>
        </div>
      </div>
