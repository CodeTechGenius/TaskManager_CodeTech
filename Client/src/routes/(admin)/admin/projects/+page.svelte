<script>
  ...
  async function loadProjects() {
    ...
    try {
      ...
      let result = await api.getProjects({params})
      // DB -> UI mapping
      projects = (result.data || []).map((p) => ({
        ...
        status: p.status,
        progress: 0, // TODO: replace when tasks table exists
        priority: p.priority || "medium",
        tech: Array.isArray(p.tech) ? p.tech : [],
        team: Array.isArray(p.team) ? p.team : [],
        dueDate: p.estimated_end_date, // used by your UI
        startDate: p.start_date,
        budget: Number(p.total_budget || 0),
        spent: 0, // optional
        client: p.client_name || "—",
        tasks: { total: 0, completed: 0, inProgress: 0, pending: 0 }, // TODO later
        category: p.category || "—",
        icon: "📁", // optional: you can map by category
      }));

      // Use server stats directly
...
function updateFilterCounts() {
  ...
  $: (async () => {
  // reload from server on filter change
  await loadProjects();
})();
...
>
  <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        >
          <div class="space-y-3">
            <div class="flex items-center gap-3">
              <div
                class="w-14 h-14 lg:w-16 lg:h-16 rounded-2xl bg-white/10 backdrop-blur-md border-2 border-white/30 flex items-center justify-center shadow-2xl"
              >
              ...
              <div>
                ...
                <p
                  class="text-primary-100 text-sm lg:text-base font-medium mt-1"
                >
            ...
            <div class="flex flex-wrap gap-3 mt-4">
              <div
                class="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-xl border border-white/20"
              >
              ...
              >
                <div
                  class="w-2 h-2 bg-success-400 rounded-full animate-pulse"
                ></div>
                <span class="text-sm font-semibold"
                  >{stats.activeProjects} Active</span
                >
              </div>
              <div
                class="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-xl border border-white/20"
              >
                <span class="text-sm font-semibold"
                  >{stats.averageProgress}% Avg Progress</span
                >
              </div>
            </div>
          </div>
          ...
          >
            <svg
              class="w-5 h-5 lg:w-6 lg:h-6 group-hover:rotate-90 transition-transform duration-300"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
    ...
    {:else}
      ...
      <div
        class="bg-white dark:bg-neutral-800 rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 lg:p-6"
      >
        <button
        ...
        >
          ...
          <svg
            class="w-5 h-5 transition-transform {showFilterPanel
              ? 'rotate-180'
              : ''}"
            fill="none"
            stroke="currentColor"
        ...
        <div class="space-y-4 {showFilterPanel ? 'block' : 'hidden lg:block'}">
          ...
          <div class="flex flex-wrap gap-2">
            {#each statusFilters as filter}
              <button
                ...
                class="px-4 py-2 rounded-xl font-bold text-sm transition-all {selectedFilter ===
                filter.id
              ...
              >
                {filter.label}
                <span
                  class="ml-2 px-2 py-0.5 rounded-full bg-white dark:bg-neutral-800 text-xs font-black"
                >
                  {filter.count}
                </span>
              </button>
          ...
          <div
            class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-700"
          >
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                class="p-2.5 rounded-xl transition-all {viewMode === 'grid'
              ...
              <button
                ...
                class="p-2.5 rounded-xl transition-all {viewMode === 'list'
                  ? 'bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400'
                  : 'bg-neutral-100 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-600'}"
      ...
      {#if filteredProjects.length === 0}
        <div
          class="flex flex-col items-center justify-center py-20 bg-white dark:bg-neutral-800 rounded-3xl border-2 border-dashed border-neutral-300 dark:border-neutral-700"
        >
          <div
            class="w-24 h-24 bg-neutral-100 dark:bg-neutral-700 rounded-full flex items-center justify-center mb-6"
          >
            <svg
          ...
          <h3 class="text-xl font-black text-neutral-900 dark:text-white mb-2">
            No projects found
          </h3>
              ...
              <div class="p-6 space-y-4">
                <div class="flex items-start justify-between gap-3">
                  <div class="flex items-center gap-3 flex-1 min-w-0">
                    <div
                      class="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-2xl shadow-lg flex-shrink-0"
                    >
                      {project.icon}
                    </div>
                    <div class="flex-1 min-w-0">
                      <h3
                        class="text-lg font-black text-neutral-900 dark:text-white truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors"
                      >
                        {project.name}
                      </h3>
                      <p
                        class="text-xs text-neutral-600 dark:text-neutral-400 font-semibold truncate"
                      >
                        {project.client}
                      </p>
                    </div>
                  ...
                  <span
                    class="px-3 py-1 rounded-full text-xs font-bold {getPriorityColor(
                      project.priority,
                    )} flex-shrink-0"
                  >
                ...
                <div>
                  ...
                  <div
                    class="relative w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-3 overflow-hidden"
                  >
                    <div
                      class="absolute inset-y-0 left-0 bg-gradient-to-r {getProgressColor(
                        project.progress,
                      )} rounded-full transition-all duration-500 flex items-center justify-end pr-1"
                ...
                <div class="flex flex-wrap gap-1.5">
                  {#each project.tech.slice(0, 3) as tech}
                    <span
                      class="px-2 py-1 bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded-lg text-xs font-semibold"
                    >
                      {tech}
                    </span>
                  {/each}
                  {#if project.tech.length > 3}
                    <span
                      class="px-2 py-1 bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded-lg text-xs font-bold"
                    >
                ...
                >
                  <div class="flex items-center -space-x-2">
                    ...
                    {#if project.team.length > 4}
                      <div
                        class="w-8 h-8 rounded-full bg-neutral-200 dark:bg-neutral-700 border-2 border-white dark:border-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-400 text-xs font-bold"
                      >
                  ...
                  <span
                    class="px-3 py-1 rounded-full text-xs font-bold border-2 {getStatusColor(
                      project.status,
                    )}"
                  >
                ...
                <div
                  class="grid grid-cols-3 gap-3 pt-3 border-t border-neutral-200 dark:border-neutral-700"
                >
                  <div class="text-center">
                    <p
                      class="text-lg font-black text-neutral-900 dark:text-white"
                    >
                  ...
                  <div class="text-center">
                    <p
                      class="text-lg font-black text-info-600 dark:text-info-400"
                    >
                  ...
                  <div class="text-center">
                    <p
                      class="text-lg font-black text-warning-600 dark:text-warning-400"
                    >
                ...
                <div class="flex items-center justify-between pt-2">
                  ...
                  {#if getDaysRemaining(project.dueDate) > 0}
                    <span
                      class="text-xs font-bold {getDaysRemaining(
                        project.dueDate,
                      ) <= 7
