<script>
  ...
  async function loadEmployeeData() {
    try {
      ...
      employee = {
        ...
        bio: 'Full-stack developer with 8+ years of experience in building scalable web applications. Passionate about clean code, modern architectures, and mentoring junior developers.',
        skills: ['React', 'Node.js', 'Python', 'AWS', 'Docker', 'MongoDB', 'SvelteKit', 'TypeScript', 'PostgreSQL', 'Redis'],
      ...
      // Mock activities
      activities = [
        { id: 1, type: 'task_completed', title: 'Completed "Design Homepage UI"', project: 'E-commerce Platform', timestamp: '2 hours ago', icon: '✅' },
        { id: 2, type: 'comment', title: 'Commented on "Payment Gateway"', project: 'E-commerce Platform', timestamp: '5 hours ago', icon: '💬' },
        { id: 3, type: 'task_created', title: 'Created "API Documentation"', project: 'CRM Dashboard', timestamp: '1 day ago', icon: '📝' },
        { id: 4, type: 'project_joined', title: 'Joined "Healthcare Portal"', project: 'Healthcare Portal', timestamp: '2 days ago', icon: '🎯' },
        { id: 5, type: 'task_updated', title: 'Updated "User Authentication"', project: 'Mobile Banking App', timestamp: '3 days ago', icon: '🔄' }
      ];

      // Mock tasks
      tasks = [
        { id: 1, title: 'Payment Gateway Integration', project: 'E-commerce Platform', priority: 'urgent', status: 'in-progress', dueDate: '2026-01-11', progress: 75 },
        { id: 2, title: 'Database Optimization', project: 'CRM Dashboard', priority: 'high', status: 'in-progress', dueDate: '2026-01-15', progress: 45 },
        { id: 3, title: 'User Authentication Module', project: 'Healthcare Portal', priority: 'high', status: 'in-progress', dueDate: '2026-01-18', progress: 90 },
        { id: 4, title: 'API Documentation', project: 'CRM Dashboard', priority: 'medium', status: 'todo', dueDate: '2026-01-20', progress: 30 },
        { id: 5, title: 'Code Review & Testing', project: 'E-commerce Platform', priority: 'high', status: 'completed', dueDate: '2026-01-08', progress: 100 }
      ];

      // Mock projects
      projects = [
        { id: 1, name: 'E-commerce Platform', icon: '🛒', role: 'Lead Developer', status: 'active', tasks: 8, completedTasks: 24, progress: 75, deadline: '2026-03-15' },
        { id: 2, name: 'Mobile Banking App', icon: '📱', role: 'Security Specialist', status: 'active', tasks: 10, completedTasks: 45, progress: 60, deadline: '2026-04-20' },
        { id: 3, name: 'Healthcare Portal', icon: '🏥', role: 'Full Stack Developer', status: 'active', tasks: 6, completedTasks: 55, progress: 85, deadline: '2026-02-28' }
      ];

      // Mock timesheet
      timesheet = [
        { date: '2026-01-10', hours: 8, task: 'Payment Gateway Integration', project: 'E-commerce Platform' },
        { date: '2026-01-09', hours: 7.5, task: 'Database Optimization', project: 'CRM Dashboard' },
        { date: '2026-01-08', hours: 8, task: 'Code Review & Testing', project: 'E-commerce Platform' },
        { date: '2026-01-07', hours: 6, task: 'User Authentication Module', project: 'Healthcare Portal' },
        { date: '2026-01-06', hours: 8, task: 'API Documentation', project: 'CRM Dashboard' }
      ];
  ...
  function getTaskStatusColor(status) {
    const colors = {
      ...
      'in-progress': 'bg-info-100 text-info-700 dark:bg-info-900/30 dark:text-info-400',
      'todo': 'bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300',
      'blocked': 'bg-error-100 text-error-700 dark:bg-error-900/30 dark:text-error-400'
    };
  ...
  {#if loading}
    <div class="flex items-center justify-center min-h-screen">
      <div class="flex flex-col items-center gap-4">
        ...
        <p class="text-neutral-600 dark:text-neutral-400 font-bold text-sm sm:text-lg">Loading profile...</p>
      </div>
    </div>
  {:else if employee}
    <div class="max-w-7xl mx-auto p-3 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      <div class="relative rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden">
        <div class="absolute inset-0 bg-gradient-to-r from-primary-600 via-primary-700 to-primary-8c00"></div>
        ...
        <div class="relative p-4 sm:p-6 lg:p-10 text-white">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 flex-1">
              ...
              <div class="relative group">
                {#if employee.photoUrl}
                  <img src={employee.photoUrl} alt={employee.name} class="w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32 rounded-3xl object-cover shadow-2xl border-4 border-white/30" />
                {:else}
                  <div class="w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32 rounded-3xl bg-white/10 backdrop-blur-md border-4 border-white/30 flex items-center justify-center shadow-2xl text-4xl sm:text-5xl lg:text-6xl font-black">
                    {employee.avatar}
                  </div>
                {/if}
                <div class="absolute bottom-0 right-0 w-7 h-7 sm:w-8 sm:h-8 {getAvailabilityColor(employee.availability)} rounded-full border-4 border-white/30"></div>
              </div>
              
              <!-- Info -->
              <div class="flex-1">
                <div class="flex flex-wrap items-start gap-3 mb-2">
                  <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight">{employee.name}</h1>
                  <span class="px-3 py-1 rounded-full text-xs sm:text-sm font-bold border-2 {getStatusColor(employee.status)}">
                    {employee.status.replace('-', ' ').toUpperCase()}
                  </span>
                </div>
                <p class="text-base sm:text-lg font-bold text-primary-100 mb-1">{employee.role}</p>
                <p class="text-xs sm:text-sm font-semibold text-primary-200 mb-3">{employee.department}</p>
                
                <div class="flex flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm">
                  <div class="flex items-center gap-1.5 sm:gap-2 bg-white/10 backdrop-blur-md px-2 sm:px-3 py-1.5 rounded-xl border border-white/20">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
            <div class="text-center p-3 sm:p-4 bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/20">
              <p class="text-2xl sm:text-3xl font-black">{employee.currentTasks}</p>
              <p class="text-xs font-semibold text-white/80 mt-1">Active Tasks</p>
            </div>
            <div class="text-center p-3 sm:p-4 bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/20">
              <p class="text-2xl sm:text-3xl font-black">{employee.completedTasks}</p>
              <p class="text-xs font-semibold text-white/80 mt-1">Completed</p>
            </div>
            <div class="text-center p-3 sm:p-4 bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/20">
              <p class="text-2xl sm:text-3xl font-black">{employee.projects.length}</p>
              <p class="text-xs font-semibold text-white/80 mt-1">Projects</p>
            </div>
            <div class="text-center p-3 sm:p-4 bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/20">
              <p class="text-2xl sm:text-3xl font-black">{Math.round((employee.completedTasks / employee.totalTasks) * 100)}%</p>
              <p class="text-xs font-semibold text-white/80 mt-1">Success Rate</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Tabs -->
      <div class="bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl shadow-lg border-2 border-neutral-200 dark:border-neutral-700 p-2">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-6">
          {#if activeTab === 'overview'}
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
              <h3 class="text-lg sm:text-xl font-black text-neutral-900 dark:text-white mb-3 sm:mb-4 flex items-center gap-2">
                <span class="text-xl sm:text-2xl">📝</span>
                About
              </h3>
              <p class="text-sm sm:text-base text-neutral-700 dark:text-neutral-300 leading-relaxed">{employee.bio}</p>
            </div>

            <!-- Expertise Card -->
            <div class="bg-gradient-to-br from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-primary-200 dark:border-primary-800 p-4 sm:p-6">
              <h3 class="text-lg sm:text-xl font-black text-primary-900 dark:text-primary-100 mb-3 flex items-center gap-2">
                <span class="text-xl sm:text-2xl">🎯</span>
                Expertise & Experience
              </h3>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <p class="text-xs font-semibold text-primary-600 dark:text-primary-500 mb-1">Primary Expertise</p>
                  <p class="text-base sm:text-lg font-black text-primary-800 dark:text-primary-300">{employee.expertise}</p>
                </div>
                <div>
                  <p class="text-xs font-semibold text-primary-600 dark:text-primary-500 mb-1">Experience</p>
                  <p class="text-base sm:text-lg font-black text-primary-800 dark:text-primary-300">{employee.experience}</p>
                </div>
                <div>
                  <p class="text-xs font-semibold text-primary-600 dark:text-primary-500 mb-1">Team Size</p>
                  <p class="text-base sm:text-lg font-black text-primary-800 dark:text-primary-300">{employee.teamSize} Members</p>
                </div>
                <div>
                  <p class="text-xs font-semibold text-primary-600 dark:text-primary-500 mb-1">Reporting To</p>
                  <p class="text-base sm:text-lg font-black text-primary-800 dark:text-primary-300">{employee.reportingTo}</p>
                </div>
              </div>
            </div>
          ...
          {:else if activeTab === 'tasks'}
            <!-- Tasks List -->
            <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
              <h3 class="text-lg sm:text-xl font-black text-neutral-900 dark:text-white mb-4 flex items-center gap-2">
                <span class="text-xl sm:text-2xl">✅</span>
                ...
                {#each tasks as task}
                  ...
                  >
                    <div class="flex items-start justify-between gap-3 mb-2">
                      <h4 class="font-bold text-neutral-900 dark:text-white flex-1 text-sm sm:text-base">{task.title}</h4>
                      <div class="flex gap-2 flex-shrink-0">
                        <span class="px-2 py-1 rounded-lg text-xs font-bold {getPriorityColor(task.priority)}">
                          {task.priority.toUpperCase()}
                        </span>
                        <span class="px-2 py-1 rounded-lg text-xs font-bold {getTaskStatusColor(task.status)}">
                          {task.status.replace('-', ' ').toUpperCase()}
                        </span>
                      </div>
                    </div>
                    <p class="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400 font-semibold mb-2">{task.project}</p>
                    ...
                    <p class="text-xs text-neutral-500 dark:text-neutral-500 mt-2">Due: {formatDate(task.dueDate)}</p>
                  </button>
                {/each}
              </div>
            </div>
                  ...
                  >
                    <div class="flex items-center gap-3 mb-3">
                      ...
                      <div class="flex-1 text-left">
                        <h4 class="font-bold text-neutral-900 dark:text-white text-sm sm:text-base">{project.name}</h4>
                        <p class="text-xs text-neutral-600 dark:text-neutral-400 font-semibold">{project.role} • {project.tasks} active tasks</p>
                      </div>
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
              ...
              <div class="space-y-3">
                {#each activities as activity}
                  <div class="flex items-start gap-3 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
            ...
            <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
              ...
              <div class="space-y-3">
                {#each timesheet as entry}
                  <div class="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                    <div class="flex items-center justify-between mb-2">
                      <p class="text-sm font-bold text-neutral-900 dark:text-white">{formatDate(entry.date)}</p>
                      <span class="px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded-lg text-xs font-black">
                        {entry.hours} hrs
                      </span>
                    </div>
                    <p class="text-sm font-semibold text-neutral-700 dark:text-neutral-300">{entry.task}</p>
                    <p class="text-xs text-primary-600 dark:text-primary-400 font-semibold mt-1">{entry.project}</p>
                  </div>
                {/each}
              </div>
              <div class="mt-4 p-4 rounded-xl bg-gradient-to-br from-success-50 to-success-100 dark:from-success-900/20 dark:to-success-800/20 border-2 border-success-200 dark:border-success-800">
                <p class="text-sm font-semibold text-success-700 dark:text-success-400 mb-1">Total Hours This Week</p>
                <p class="text-3xl font-black text-success-800 dark:text-success-300">
        ...
        <div class="space-y-4 sm:space-y-6">
          <!-- Quick Info -->
          <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
            <h3 class="text-base sm:text-lg font-black text-neutral-900 dark:text-white mb-4">Quick Info</h3>
            <div class="space-y-3">
              <div>
                <p class="text-xs font-semibold text-neutral-600 dark:text-neutral-400 mb-1">Login ID</p>
                <p class="text-sm font-bold text-neutral-900 dark:text-white">{employee.loginId}</p>
              </div>
              <div>
                <p class="text-xs font-semibold text-neutral-600 dark:text-neutral-400 mb-1">Join Date</p>
                <p class="text-sm font-bold text-neutral-900 dark:text-white">{formatDate(employee.joinDate)}</p>
              </div>
              <div>
                <p class="text-xs font-semibold text-neutral-600 dark:text-neutral-400 mb-1">Salary</p>
                <p class="text-sm font-bold text-neutral-900 dark:text-white">{employee.salary}</p>
              </div>
              <div>
                <p class="text-xs font-semibold text-neutral-600 dark:text-neutral-400 mb-1">Last Active</p>
                <p class="text-sm font-bold text-neutral-900 dark:text-white">{employee.lastActive}</p>
              </div>
            </div>
          </div>
          ...
          {#if employee.social}
            <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
              <h3 class="text-base sm:text-lg font-black text-neutral-900 dark:text-white mb-4">Social Links</h3>
              <div class="space-y-3">
          ...
          <!-- Performance -->
          <div class="bg-gradient-to-br from-success-50 to-success-100 dark:from-success-900/20 dark:to-success-800/20 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-success-200 dark:border-success-800 p-4 sm:p-6">
            <h3 class="text-base sm:text-lg font-black text-success-900 dark:text-success-100 mb-4">Performance</h3>
            <div class="space-y-3">
              <div>
                <div class="flex justify-between mb-2">
                  <span class="text-xs font-semibold text-success-700 dark:text-success-400">Task Completion</span>
                  <span class="text-xs font-black text-success-800 dark:text-success-300">{Math.round((employee.completedTasks / employee.totalTasks) * 100)}%</span>
                </div>
                <div class="w-full bg-success-200 dark:bg-success-900/30 rounded-full h-2">
                  <div
              ...
              <div>
                <div class="flex justify-between mb-2">
                  <span class="text-xs font-semibold text-success-700 dark:text-success-400">Project Progress</span>
                  <span class="text-xs font-black text-success-800 dark:text-success-300">73%</span>
                </div>
                <div class="w-full bg-success-200 dark:bg-success-900/30 rounded-full h-2">
                  <div
                    class="bg-gradient-to-r from-success-500 to-success-600 h-2 rounded-full"
  ...
  {:else}
    <div class="flex items-center justify-center min-h-screen">
      <div class="text-center">
        <h3 class="text-2xl font-black text-neutral-900 dark:text-white mb-2">Employee Not Found</h3>
        <p class="text-neutral-600 dark:text-neutral-400 mb-6">The employee you're looking for doesn't exist.</p>
        <button
          on:click={goBack}
...
{#if showEditModal}
  <div class="fixed inset-0 bg-neutral-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto" transition:fade>
    <div class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-2xl border-2 border-neutral-200 dark:border-neutral-700 w-full max-w-3xl my-8" in:scale={{ duration: 300 }}>
      <!-- Modal content similar to team page -->
      <div class="flex items-center justify-between p-4 sm:p-6 border-b border-neutral-200 dark:border-neutral-700">
        <h3 class="text-xl sm:text-2xl font-black text-neutral-900 dark:text-white">Edit Employee</h3>
        <button on:click={closeEditModal} class="p-2 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-all">
      ...
      <div class="p-4 sm:p-6 max-h-[70vh] overflow-y-auto">
        <p class="text-center text-neutral-600 dark:text-neutral-400">Edit form (same as add employee form)</p>
