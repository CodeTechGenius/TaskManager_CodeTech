<script>
  ...
  function calculateStats() {
    ...
    stats.activeMembers = teamMembers.filter(
      (m) => m.status === "active",
    ).length;
    stats.departments = [
  ...
  function updateFilterCounts() {
    ...
    roleFilters[1].count = teamMembers.filter(
      (m) => m.roleType === "developer",
    ).length;
    roleFilters[2].count = teamMembers.filter(
      (m) => m.roleType === "designer",
    ).length;
    roleFilters[3].count = teamMembers.filter(
      (m) => m.roleType === "manager",
    ).length;
    roleFilters[4].count = teamMembers.filter(
      (m) => m.roleType === "qa",
    ).length;
    roleFilters[5].count = teamMembers.filter(
      (m) => m.roleType === "devops",
    ).length;
  }
    ...
    try {
      ...
      if (showEditModal) {
        ...
        const newEmployee = {
          ...
          expertise: employeeForm.expertise,
          experience: employeeForm.experience,
          salary: Number(employeeForm.salary.replace(/[₹,]/g, "")),
          currentTasks: 0,
          completedTasks: 0,
          projects: [],
...
>
  <div class="max-w-7xl mx-auto p-3 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
    ...
    <div class="relative rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden">
      <div
        class="absolute inset-0 bg-gradient-to-r from-primary-600 via-primary-700 to-primary-800"
      ></div>
      <div
      ...
      <div class="relative p-4 sm:p-6 lg:p-10 text-white">
        <div
          class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6"
        >
          <div class="space-y-3">
            <div class="flex items-center gap-2 sm:gap-3">
              <div
                class="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-md border-2 border-white/30 flex items-center justify-center shadow-2xl"
              >
              ...
              <div>
                ...
                <p
                  class="text-primary-100 text-xs sm:text-sm lg:text-base font-medium mt-1"
                >
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
              <div
                class="flex items-center gap-1.5 sm:gap-2 bg-white/10 backdrop-blur-md px-2 sm:px-4 py-2 rounded-lg sm:rounded-xl border border-white/20"
              >
                <span class="text-lg sm:text-2xl font-black"
                  >{stats.totalMembers}</span
                >
              ...
              >
                <div
                  class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-success-400 rounded-full animate-pulse"
                ></div>
                <span class="text-xs sm:text-sm font-semibold"
                  >{stats.activeMembers} Active</span
                >
              </div>
              <div
                class="flex items-center gap-1.5 sm:gap-2 bg-white/10 backdrop-blur-md px-2 sm:px-4 py-2 rounded-lg sm:rounded-xl border border-white/20"
              ...
              >
                <span class="text-xs sm:text-sm font-semibold"
                  >{stats.averageTasksPerMember} Avg</span
                >
              </div>
            </div>
          </div>
          ...
          >
            <svg
              class="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 group-hover:rotate-90 transition-transform duration-300"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
    ...
    {:else}
      ...
      <div
        class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-xl border-2 border-neutral-200 dark:border-neutral-700 p-3 sm:p-4 lg:p-6"
      >
        <button
        ...
        >
          ...
          <svg
            class="w-4 h-4 sm:w-5 sm:h-5 transition-transform {showFilterPanel
              ? 'rotate-180'
              : ''}"
            fill="none"
            stroke="currentColor"
        ...
        <div
          class="space-y-3 sm:space-y-4 {showFilterPanel
            ? 'block'
            : 'hidden lg:block'}"
        >
          <!-- Search Bar -->
          <div class="relative">
            ...
            >
              <svg
                class="w-4 h-4 sm:w-5 sm:h-5 text-neutral-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div class="flex flex-wrap gap-1.5 sm:gap-2">
            {#each roleFilters as filter}
              <button
                ...
                class="px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm transition-all {selectedRole ===
                filter.id
              ...
              >
                {filter.label}
                <span
                  class="ml-1.5 sm:ml-2 px-1.5 sm:px-2 py-0.5 rounded-full bg-white dark:bg-neutral-800 text-xs font-black"
                >
                  {filter.count}
                </span>
              </button>
          ...
          <div
            class="flex flex-col sm:flex-row gap-2 sm:gap-3 items-start sm:items-center justify-between pt-3 sm:pt-4 border-t border-neutral-200 dark:border-neutral-700"
          >
            <div
              class="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto"
            >
              <label
                class="text-xs sm:text-sm font-bold text-neutral-700 dark:text-neutral-300"
                >Dept:</label
              >
              <select
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                class="p-2 rounded-lg sm:rounded-xl transition-all {viewMode ===
                'grid'
      ...
      {#if searchQuery || selectedRole !== "all" || selectedDepartment !== "all"}
        <div class="flex items-center justify-between px-2">
          <p
            class="text-xs sm:text-sm font-bold text-neutral-600 dark:text-neutral-400"
          >
      ...
      {#if filteredMembers.length === 0}
        <div
          class="flex flex-col items-center justify-center py-16 sm:py-20 bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl border-2 border-dashed border-neutral-300 dark:border-neutral-700"
        >
          <div
            class="w-16 sm:w-24 h-16 sm:h-24 bg-neutral-100 dark:bg-neutral-700 rounded-full flex items-center justify-center mb-4 sm:mb-6"
          >
            <svg
              class="w-8 sm:w-12 h-8 sm:h-12 text-neutral-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <h3
            class="text-lg sm:text-xl font-black text-neutral-900 dark:text-white mb-2"
          >
            No members found
          </h3>
          <p
            class="text-sm sm:text-base text-neutral-600 dark:text-neutral-400 mb-4 sm:mb-6 text-center px-4"
          >
            Try adjusting filters or add new member
          </p>
          <button
            on:click={openAddModal}
            ...
            >
              <div class="p-4 sm:p-6">
                  ...
                  <div class="relative">
                    {#if member.photoUrl}
                      <img
                        src={member.photoUrl}
                        alt={member.name}
                        class="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl object-cover shadow-lg"
                      />
                    {:else}
                      <div
                        class="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-secondary-400 to-secondary-600 flex items-center justify-center text-white text-xl sm:text-2xl font-black shadow-lg"
                      >
                        {member.avatar}
                      </div>
                    {/if}
                    <div
                      class="absolute -bottom-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 {getAvailabilityColor(
                        member.availability,
                      )} rounded-full border-4 border-white dark:border-neutral-800"
                    ></div>
                  </div>
                ...
                <div class="mb-3 sm:mb-4">
                  <h3
                    class="text-base sm:text-xl font-black text-neutral-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors mb-1 truncate"
                  >
                    {member.name}
                  </h3>
                  <p
                    class="text-xs sm:text-sm font-bold text-neutral-600 dark:text-neutral-400 truncate"
                  >
                    {member.role}
                  </p>
                  <div class="flex items-center gap-2 mt-2">
                    <span
                      class="px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs font-bold border-2 {getStatusColor(
                        member.status,
                      )}"
                    >
                    ...
                    <p
                      class="text-xs text-neutral-500 dark:text-neutral-500 font-medium truncate"
                    >
                      {member.department}
                    </p>
                  </div>
                ...
                <!-- Expertise -->
                <div
                  class="mb-3 sm:mb-4 p-2 sm:p-3 rounded-xl bg-gradient-to-br from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 border border-primary-200 dark:border-primary-800"
                >
                  <p
                  ...
                  <p
                    class="text-sm font-black text-primary-700 dark:text-primary-400"
                  >
                    {member.expertise}
                  </p>
                  <p
                    class="text-xs text-primary-600 dark:text-primary-500 font-semibold mt-1"
                  >
                    {member.experience}
                  </p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-4">
                  <div
                    class="p-2 sm:p-3 rounded-xl bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800"
                  >
                    <p
                      class="text-lg sm:text-2xl font-black text-primary-700 dark:text-primary-400"
                    >
                      {member.currentTasks}
                    </p>
                    <p
                      class="text-xs font-semibold text-primary-600 dark:text-primary-500"
                    >
                      Current
                    </p>
                  </div>
                  <div
                    class="p-2 sm:p-3 rounded-xl bg-success-50 dark:bg-success-900/20 border border-success-200 dark:border-success-800"
                  >
                    <p
                      class="text-lg sm:text-2xl font-black text-success-700 dark:text-success-400"
                    >
                      {member.completedTasks}
                    </p>
                    <p
                      class="text-xs font-semibold text-success-600 dark:text-success-500"
                    >
                      Done
                    </p>
                  </div>
                ...
                <div class="mb-3 sm:mb-4">
                  <p
                    class="text-xs font-semibold text-neutral-600 dark:text-neutral-400 mb-2"
                  >
                    Skills
                  </p>
                  <div class="flex flex-wrap gap-1.5">
                    {#each member.skills.slice(0, 3) as skill}
                      <span
                        class="px-2 py-0.5 sm:py-1 bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded-lg text-xs font-semibold"
                      >
                        {skill}
                      </span>
                    {/each}
                    {#if member.skills.length > 3}
                      <span
                        class="px-2 py-0.5 sm:py-1 bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded-lg text-xs font-bold"
                      >
                ...
                <!-- Contact Info -->
                <div
                  class="pt-3 sm:pt-4 border-t border-neutral-200 dark:border-neutral-700 space-y-1.5 sm:space-y-2"
                >
                  <div class="flex items-center gap-2 text-xs">
                    <svg
                      class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-neutral-400 flex-shrink-0"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <span
                      class="font-semibold text-neutral-600 dark:text-neutral-400 truncate"
                      >{member.email}</span
                    >
                  </div>
                  <div class="flex items-center gap-2 text-xs">
                ...
                <div class="flex gap-3 sm:gap-4">
                  ...
                  <div class="relative flex-shrink-0">
                    {#if member.photoUrl}
                      <img
                        src={member.photoUrl}
                        alt={member.name}
                        class="w-14 h-14 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl object-cover shadow-lg"
                      />
                    {:else}
                      <div
                        class="w-14 h-14 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-gradient-to-br from-secondary-400 to-secondary-600 flex items-center justify-center text-white text-lg sm:text-xl font-black shadow-lg"
                      >
                        {member.avatar}
                      </div>
                    {/if}
                    <div
                      class="absolute -bottom-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 {getAvailabilityColor(
                        member.availability,
                      )} rounded-full border-3 sm:border-4 border-white dark:border-neutral-800"
                    ></div>
                  </div>
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 mb-2">
                      <div class="flex-1 min-w-0">
                        <h3
                          class="text-base sm:text-xl font-black text-neutral-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors truncate"
                        >
                          {member.name}
                        </h3>
                        <p
                    ...
                    <div class="space-y-2 mb-3">
                      <div class="flex items-center gap-1.5 sm:gap-2 text-xs">
                        <span
                          class="px-2 py-0.5 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded font-bold"
                        >
                          {member.expertise}
                        </span>
                        <span
                          class="text-neutral-500 dark:text-neutral-500 font-medium"
                      ...
                      <div class="flex flex-wrap gap-1">
                        {#each member.skills.slice(0, 2) as skill}
                          <span
                            class="px-1.5 py-0.5 bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded text-xs font-semibold"
                          >
                            {skill}
                          </span>
                        {/each}
                        {#if member.skills.length > 2}
                          <span
                            class="px-1.5 py-0.5 bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 rounded text-xs font-bold"
                          >
                    ...
                    <!-- Stats and Actions -->
                    <div
                      class="flex items-center justify-between gap-2 pt-2 sm:pt-3 border-t border-neutral-200 dark:border-neutral-700"
                    >
                      <div class="flex gap-3 sm:gap-4 text-xs">
                        <div>
                          <span
                            class="font-black text-primary-700 dark:text-primary-400"
                            >{member.currentTasks}</span
                          >
                          <span
                            class="text-neutral-600 dark:text-neutral-400 font-medium ml-1"
                            >Active</span
                          >
                        </div>
                        <div>
                          <span
                            class="font-black text-success-700 dark:text-success-400"
                            >{member.completedTasks}</span
                          >
                          <span
                            class="text-neutral-600 dark:text-neutral-400 font-medium ml-1"
                            >Done</span
...
{#if showAddModal || showEditModal}
  <div
    class="fixed inset-0 bg-neutral-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4"
    transition:fade
  >
    <div
      class="bg-white dark:bg-neutral-800 rounded-2xl sm:rounded-3xl shadow-2xl border-2 border-neutral-200 dark:border-neutral-700 w-full max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col"
      in:scale={{ duration: 300 }}
    >
      <!-- Modal Header -->
      <div
        class="flex items-center justify-between p-4 sm:p-6 border-b border-neutral-200 dark:border-neutral-700 flex-shrink-0"
      >
        <h3
          class="text-xl sm:text-2xl font-black text-neutral-900 dark:text-white flex items-center gap-2"
        >
      ...
      <div class="overflow-y-auto flex-1 p-4 sm:p-6">
        ...
        >
          ...
          <div>
            <label
              class="block text-sm font-bold text-neutral-900 dark:text-white mb-3"
              >Profile Photo</label
            >
            <div class="flex items-center gap-4">
              <div
                class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gradient-to-br from-secondary-400 to-secondary-600 flex items-center justify-center text-white text-2xl sm:text-3xl font-black shadow-lg overflow-hidden"
              >
                {#if avatarPreview}
                  <img
                    src={avatarPreview}
              ...
              <div class="flex-1">
                ...
                <p
                  class="text-xs text-neutral-600 dark:text-neutral-400 mt-2 font-medium"
                >
                  JPG, PNG. Max 5MB
                </p>
                {#if formErrors.photo}
                  <p
                    class="text-error-600 dark:text-error-400 text-xs font-semibold mt-1"
                  >
                    {formErrors.photo}
                  </p>
                {/if}
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label
                for="name"
                class="block text-sm font-bold text-neutral-900 dark:text-white mb-2"
              >
              ...
              <input
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.name
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-medium"
              />
              {#if formErrors.name}
                <p
                  class="text-error-600 dark:text-error-400 text-xs font-semibold mt-1"
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.email
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-medium"
              />
              {#if formErrors.email}
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.password
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-medium"
              />
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.phone
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-medium"
              />
              {#if formErrors.phone}
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.role
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-semibold"
              >
              ...
              <select
                ...
                on:change={() => {
                  ...
                  employeeForm.department =
                    depts[employeeForm.departmentType] || "";
                }}
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.department
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-semibold"
              >
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.expertise
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-semibold"
              >
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-4 py-2.5 sm:py-3 bg-neutral-50 dark:bg-neutral-900 border-2 {formErrors.experience
                  ? 'border-error-500'
                  : 'border-neutral-200 dark:border-neutral-700'} rounded-xl text-sm sm:text-base text-neutral-900 dark:text-white focus:outline-none focus:ring-4 focus:ring-primary-500/30 focus:border-primary-500 transition-all font-medium"
              />
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              {#each employeeForm.skills as skill}
                <span
                  class="px-3 py-1.5 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded-lg text-sm font-bold flex items-center gap-2"
                >
                  {skill}
                  <button
                    type="button"
...
{#if showSuccessMessage}
  ...
  >
    <div
      class="bg-success-600 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 max-w-sm"
    >
      <svg
        class="w-6 h-6"
        fill="none"
